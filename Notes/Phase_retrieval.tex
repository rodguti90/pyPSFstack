...
\title{Characterizing birefringent pupils in widefield fluorescence microscopy using phase retrieval}
...
If the Birefringent pupil is real (up to a global phase) then one needs to choose
carefully the polarization diversity in order to guarantee that
teh appropriate superpositions are formed in order to get the phase relation 
between all the compopnents. For example, when using a qwp we cannot dostinguish the
phase betweent the two columns but if we used instead a hwp then we can.
